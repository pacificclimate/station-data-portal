...
The Station Data Portal provides observations of weather and climate 
variables (such as temperature and rainfall amounts) for various regions 
in Canada. At present there are two such regions, British Columbia and
the Yukon and Northwest Territories. A separate instance of the SDP 
serves each data for each separate region. The SDP functions identically
regardless of region.

Locations of observation stations are shown on an interactive map of the 
...
based on observation date, weather element, observing agency, region and 
more.
...
A history item describes a particular configuration of a station, including its location, the variables it reports (which relate to the sensors installed in the station hardware). Many stations have only one history item, but some have several, reflecting changes of location or equipment.

The distinction between station and history is often glossed over, but it does appear in things like station markers (there is one marker per unique station location, which is derived from histories) and in the metadata available on the **Station Metadata** tab.
...
The map is the primary display of the stations you have selected. It typically takes up most of the screen real estate, which it shares with a set of tabs that include station filtering. You can adjust the proportion of the screen allocated to the map and the tabs by using the column width controls at the top of the display.
...
Note that zooming in to a particular area does not exclude stations which are out of your field of view. To select a spatially-defined subset of stations, you can draw one or more polygons on the map. To do so you must use one of the polygon drawing tools in the map toolbar on the left-hand side.

![drawing controls](./images/map-drawing-controls.png)

Using the drawing controls:

- You can draw one or more polygons on the map. Any station inside any polygon is selected.

- To draw a roughly rectangular polygon, click the **Draw a rectangle** tool icon.

  ![draw rectangle](./images/map-drawing-rectangle-icon.png)
...
- To draw a more elaborate polygon, click the **Draw a polygon** tool icon.
  ...
  ![draw polygon](./images/map-drawing-polygon-icon.png)
...
- To edit a polygon already drawn on the map, click the **Edit shapes** icon.
  ...
  ![edit shapes](./images/map-drawing-edit-icon.png)
...
- To remove one or more polygons from the map, click the **Remove shapes** icon. 
  ...
  ![remove shapes](./images/map-drawing-delete-icon.png)
...
The station filtering controls are:
...
This tab shows a count of the stations selected, and of the number of observations available from those selected stations.
...
The **Networks** tab lists details of all networks, showing their colour coding on the map.

![networks tab](./images/networks-tab.png)

## Unexpected behaviour

There are a few nuances to the aggregated data download that work the way the user expectes most of the time, but may be unexpected in a few ways.
