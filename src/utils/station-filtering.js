...
import {
  checkGeoJSONMultiPolygon,
  gJMultiPolygonBoundingBox,
...
// Note: Some consideration has been made to the possibility of adding a "strict"
// check. Which would expect observation ranges to be within the bounds, not just overlapping.
// I believe simple overlap represents closer to what a user would expect
// TODO: consider replacing hand rolled date functions with functions from date-fns
export const historyDateMatch = (history, startDate, endDate) => {
...
export const stationMatchesDates = ft.timeThis("stationMatchesDates")((
  ...
  //  actual observations.
  const r = some(
    (hx) => historyDateMatch(hx, startDate, endDate),
    station.histories,
  );
  // if (!r) {
...
export const stationInAnyNetwork = ft.timeThis("stationInAnyNetwork")((
  ...
  return contains(station.network_uri, networkUris);
});

export const atLeastOne = (items) => items.length > 0;
const variableIdsFromHistories = flow(map("variable_ids"), flatten);

export const stationReportsSomeVariables = ft.timeThis(
  "stationReportsSomeVariables",
)((station, variableIds, includeStationsWithNoObs) => {
  const stationVariableIds = ft.timeThis("stationVariableIds")(
    variableIdsFromHistories,
  )(station.histories);
...
export const stationReportsAnyFreqs = ft.timeThis("stationReportsAnyFreqs")((
  ...
  const stationFreqs = ft.timeThis("stationFreqs")(map("freq"))(
    station.histories,
  );
...
export const stationReportsClimatologyVariable = ft.timeThis(
...
export const stationInsideMultiPolygon = ft.timeThis(
...
export const climatologyVairableIdsFromVariables = flow(
  filter(({ tags }) => contains("climatology", tags)),
  map("id"),
);

export const networkUrisFromOptions = map((nw) => nw.value.uri);
...
export const stationFilter = ({
  filterValues: {
    ...
    startDate,
    endDate,
    selectedNetworks, // array of network URIs
    selectedVariables, // array of variable ids
    selectedFrequencies, // array of frequency options strings,
    onlyWithClimatology,
  },
  metadata: { variables: allVariables, stations: allStations },
}) => {
  ...
  const r = filter(
    (station) =>
      stationMatchesDates(station, startDate, endDate) &&
