...
export const stationNetwork = curry((networks, station) =>
  find({ uri: station.network_uri }, networks),
);

export const stationVariableIds = (station) =>
  flow(map("variable_ids"), unionAll)(station.histories);

export const uniqStationNames = (station) =>
  flow(map("station_name"), sortBy(identity), uniq)(station.histories);
...
export const uniqStationFreqs = (station) =>
  flow(map("freq"), uniq)(station.histories);

export const uniqStationVariableNames = curry((variables, station) =>
  flow(
    map((history) =>
      map(
